...
GENJS = '''var TPL={ dlink:{
 ...
 port_speed:'config ports {ports} speed {text}',
...
port_util:'Порт(ы) — порты для замера загрузки в процентах.',
...
port_speed:'Порт(ы) — порты; Текст — скорость: auto, 100, 1000 или «100 duplex full».',
...
port_speed:'1) выставить скорость/duplex\\n2) save\\n3) проверить, что порт поднялся на нужной скорости',
...
print('ok: gen.js v4 с портовыми операциями')

src = open('swh.py').read()
if 'gen.js?v=4' not in src:
    src = src.replace('/static/gen.js?v=3', '/static/gen.js?v=4', 1)
    print('ok: версия gen.js обновлена')
